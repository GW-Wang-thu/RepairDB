    ...
    if request.method == "POST":
        ...
        for k, v in request.POST.dict().items():
            if i < 9 and i > 0:
                ...
                common_dict.update({k: v})
            i += 1

        # 数据检查 - 2: 用户模板数据的检查
        template_id = request.POST.get('user_template')[0]  # 获取模板号
        if template_id != '':   # 如果有选择模板
            template = models.TemplateInfo.objects.filter(id=template_id).first()
        ...
        return HttpResponse(json.dumps({"status": status, "errors": errors}, ensure_ascii=False))


def rep_data_submit_select_template(request):
